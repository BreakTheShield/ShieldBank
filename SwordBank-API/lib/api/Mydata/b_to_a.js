...
const apiUrl = 'http://192.168.10.130:3000/api/mydata/send_btoa';
...
router.post('/', [validateUserToken, decryptRequest], (req, res) => {          // B은행 계좌에서 A은행 계좌로 송금하는 경우
    ...
    if (amount < 0) {          // 출금 금액이 0원보다 적은 경우
        ...
        console.log("@@@@@@@@@axios  들어가기 전", bank_code);
        axios({          // B은행에서 이체를 진행하기 위한 API로 req
            ...
            console.log("@@@@@@@@@@@shield은행에서 돈을 빠지고 넣는 부분@@@@@@@@@",data);
            if (data.data.status.code == 200) {          // B은행 계좌에서 정상적으로 출금에 성공한 경우
                Model.account.update({          // update account set balance = (balance + amount) where account_number = to_account;
                    ...
                    Model.transactions.create({          // insert into transactions (from_account, to_account, amount, sendtime, from_bankcode, to_bankcode) values (from_account, to_account, amount, sendtime, 555, to_bankcode);      
