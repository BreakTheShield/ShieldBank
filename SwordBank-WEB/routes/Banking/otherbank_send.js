...
router.get("/", checkCookie, async (req, res) => {          // 마이데이터 불러오기 계좌 송금 선택 시   
    ...
    profile(cookie).then((data) => {
           ...
           html_data += `<option value="555">SHIELDBANK</option>`
           html_data += `<option value="333">SWORDBANK</option></select><br>`
...
router.post("/post", checkCookie, function (req, res) {          // 마이데이터에서 송금하기 
    ...
    console.log("$$$$$$$$$$$$$$$$$$$$뱅크코드",req.body.bank_code);
    if(req.body.bank_code == '333') {          // 송금받는 계좌의 bank_code가 555일 때 (SHIELDBANK)
    ...
    else if(req.body.bank_code == '555') {          // 송금받는 계좌의 bank_code가 333일 때 (SWORDBANK)
